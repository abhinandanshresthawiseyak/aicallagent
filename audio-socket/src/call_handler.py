...
class CallHandler:
    def __init__(self,initial_audio_in_seconds, caller_id=uuid.uuid4(), tts_next_states=None, call_type=None, out_state=4, name=None,llm_states=None, tts_folder_location=None) -> None:
        self.name=name
        ...
        self.call_logs_url='http://192.168.89.109/call_logs/'
        self.nepali=False
            ...
            while self.run_threads:
                ...
                if not self.silence_found:
                    if self.vad_dictionary and all(i==0 for i in list(self.vad_dictionary.values())[- SILENCE_CHUNKS:]):
                        ...
                        with wave.open('/app/vad_output/'+datetime.now(pytz.timezone('Asia/Kathmandu')).strftime('%Y-%m-%d %H:%M:%S')+'___'+str(self.caller_id)+'__'+str(c)+'.wav', 'wb') as wf:
                        ...
                        requests.post(self.call_logs_url, params={'caller_id': self.caller_id,'event_type': 'VAD Saves file','event_detail':'vad_output/'+str(self.caller_id)+'__'+str(c)+'.wav' }, headers={'accept': 'application/json'})   

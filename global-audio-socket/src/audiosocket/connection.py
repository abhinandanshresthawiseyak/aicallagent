...
class Connection:
  ...
  def write_audio(self, audio):
        ...
        # Process the audio in chunks of 320 bytes
        for i in range(0, audio_length, chunk_size):
            chunk = audio[i:i + chunk_size]

            # If the chunk is smaller than 320 bytes, pad with silence (zeros)
            if len(chunk) < chunk_size:
                chunk += bytes(chunk_size - len(chunk))

            # Resample and downmix if required before writing
            if self._user_resample:
                # Convert ULAW encoded audio to linear encoding if specified
                if self._user_resample.ulaw2lin:
                    chunk = audioop.ulaw2lin(chunk, 2)

                # If the audio isn't already sampled at 8KHz, downsample it
                if self._user_resample.rate != 8000:
                    chunk, self._user_resample.ratecv_state = audioop.ratecv(
                        chunk,
                        2,
                        self._user_resample.channels,
                        self._user_resample.rate,
            ...
            # Write the processed 320-byte chunk to the tx queue
            self._tx_q.put(chunk)
